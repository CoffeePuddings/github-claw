<template>
  <div class="media-page">
    <h2>🎬 视频压缩与格式转换</h2>
    <p class="desc">支持 MP4、AVI、MKV、WebM、MOV 格式</p>
    ...
    <div v-if="uploadedFile" class="options-section">
      ...
      <div class="option-group">
        <label>压缩质量 (CRF)：</label>
        <input type="range" v-model.number="quality" min="0" max="51" />
        <span>{{ quality }} (越小质量越高)</span>
      </div>
...
.option-group select, .option-group input[type="range"] { flex: 1; }
select {
  padding: 0.5rem;
